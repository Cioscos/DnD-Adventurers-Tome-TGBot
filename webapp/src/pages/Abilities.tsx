            ...
            >
              ...
              >
                ...
                <div className="flex gap-1.5 items-center shrink-0">
                  ...
                  {ab.max_uses != null && (
                    <span className="text-xs text-dnd-text-secondary font-medium tabular-nums">
                      {ab.uses ?? 0}/{ab.max_uses}
                    </span>
                  )}
                </div>
                <span className="text-dnd-text-secondary text-xs ml-1">
                  {expanded === ab.id ? '˄' : '˅'}
                </span>
              </button>
              ...
              {expanded === ab.id && (
                <div className="px-3 pb-3 space-y-3 border-t border-white/10">
                  {ab.description ? (
                    <p className="text-xs text-dnd-text-secondary mt-2 whitespace-pre-wrap leading-relaxed border-l-2 border-amber-500/40 pl-2">
                      {ab.description}
                    </p>
                  ) : (
                    <p className="text-xs text-dnd-text-secondary/50 mt-2 italic">—</p>
                  )}
                  ...
                  <div className="flex gap-2 pt-1 border-t border-white/10">
                    <button
                      onClick={() => openEdit(ab)}
                      className="text-xs px-3 py-2 rounded-lg bg-dnd-surface text-[var(--tg-theme-text-color)] font-medium active:opacity-70"
                    >
                    ...
                    <button
                      ...
                      className="text-xs px-3 py-2 rounded-lg bg-red-500/20 text-[var(--dnd-danger)] font-medium active:opacity-70 disabled:opacity-40"
                    >
                      🗑 {t('common.delete')}
                    </button>
                  </div>
          ...
          <Card className="w-full space-y-3" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <p className="text-xs text-dnd-text-secondary mb-1">{t('character.abilities.max_uses_label')}</p>
                <input
                  type="number" min="0" value={form.max_uses}
                  onChange={(e) => setForm((f) => ({ ...f, max_uses: e.target.value }))}
                  placeholder="—"
                  className="w-full bg-dnd-surface rounded-xl px-3 py-2 text-center outline-none"
                />
              </div>
              <div className="flex-1">
                <p className="text-xs text-dnd-text-secondary mb-1">{t('character.abilities.restoration_label')}</p>
                <select
                  value={form.restoration_type}
                  onChange={(e) => setForm((f) => ({ ...f, restoration_type: e.target.value }))}
                  className="w-full bg-dnd-surface rounded-xl px-2 py-2 outline-none text-sm"
                >
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2 rounded-xl bg-dnd-gold
                           text-dnd-bg font-semibold disabled:opacity-40"
              >
                {isPending ? '...' : editingAbility ? t('common.save') : t('common.add')}
              </button>
              <button onClick={closeForm} className="flex-1 py-2 rounded-xl bg-dnd-surface">
                {t('common.cancel')}
              </button>
            </div>
          </Card>
