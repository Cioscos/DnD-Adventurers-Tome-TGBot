...
async def navigation_callback(
    ...
    if isinstance(data, InvalidCallbackData):
        await _send_error(query, "Session expired. Please use /start to restart.")
        return
    ...
    try:
        ...
        if data.action == "menu":
            ...
            await _show_item_detail(query, data)
        elif data.action == "sub_list":
            await _show_sub_list(query, data)
        else:
...
async def _show_item_list(query: Any, nav: NavAction) -> None:
    ...
    page_info = f" — Page {page + 1}" if ti.has_pagination else ""
    header = f"{emoji} *{_esc(label)}*{_esc(page_info)}\n\nSelect an item:"

    keyboard = build_list_keyboard(display, nav.type_name, page, has_next)
    await query.edit_message_text(
...
async def _show_item_detail(query: Any, nav: NavAction) -> None:
    # For union types, use the concrete type if available for formatting
    # but always fetch using the base type's detail query
    fetch_type_name = nav.type_name
    concrete = nav.concrete_type

    # If the type_name IS a concrete member of a union (e.g., "Weapon"),
    # find its parent union that has the detail_query_field
    ti = registry.get_type(fetch_type_name)
    if ti is None:
        await _send_error(query, "Unknown type.")
        return

    if ti.detail_query_field is None:
        # Might be a concrete union member — try to find the union parent
        ...
        if parent_union and parent_union.detail_query_field:
            ...
            await _send_error(query, "Cannot fetch details for this type.")
            return
    ...
    if not item:
        await _send_error(query, f"Item '{nav.index}' not found.")
        return

    # Resolve concrete type from __typename for union responses
...
async def _show_sub_list(query: Any, nav: NavAction) -> None:
    parent_ti = registry.get_type(nav.type_name)
    if parent_ti is None:
        await _send_error(query, "Unknown parent type.")
        return

    # If parent is a concrete union member, find the union
    ...
    if not display:
        await _send_error(query, "No items in this sub-category.")
        return

    # Determine the sub-type for badge display
    ...
    page_info = f" — Page {page + 1}" if has_next or page > 0 else ""
    header = f"📂 *{_esc(nice_label)}*{_esc(page_info)}\n\nSelect an item:"
